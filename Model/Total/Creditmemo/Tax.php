...
class Tax extends \Magento\Sales\Model\Order\Creditmemo\Total\Tax
{
    ...
    public function collect(\Magento\Sales\Model\Order\Creditmemo $creditmemo)
    {
        ...
        //override start
        $handlingFeeTaxAmount = 0;
        $baseHandlingFeeTaxAmount = 0;
        //override end
        $totalTax = 0;
        $baseTotalTax = 0;
        ...
        if ($invoice = $creditmemo->getInvoice()) {
            ...
            //Start Override
            if ($order->getBaseHandlingFeeAmount() && $creditmemo->getBaseHandlingFeeAmount()) {
                $handlingFeeTaxAmount     = $invoice->getHandlingFeeTaxAmount();
                $baseHandlingFeeTaxAmount = $invoice->getBaseHandlingFeeTaxAmount();
                $handlingFeeTaxAmount     = $creditmemo->roundPrice($handlingFeeTaxAmount);
                $baseHandlingFeeTaxAmount = $creditmemo->roundPrice($baseHandlingFeeTaxAmount, 'base');

                $totalTax     += $handlingFeeTaxAmount;
                $baseTotalTax += $baseHandlingFeeTaxAmount;
            }
            //End Override
        } else {
            ...
            //Start Override
            if ($order->getBaseHandlingFeeAmount() && $creditmemo->getBaseHandlingFeeAmount()) {
                $handlingFeeTaxAmount     = $order->getHandlingFeeTaxAmount();
                $baseHandlingFeeTaxAmount = $order->getBaseHandlingFeeTaxAmount();
                $handlingFeeTaxAmount     = $creditmemo->roundPrice($handlingFeeTaxAmount);
                $baseHandlingFeeTaxAmount = $creditmemo->roundPrice($baseHandlingFeeTaxAmount, 'base');
        ...
        // Overide Start. Set HandlingFeeTax
        $creditmemo->setHandlingFeeTaxAmount($handlingFeeTaxAmount);
        $creditmemo->setBaseHandlingFeeTaxAmount($baseHandlingFeeTaxAmount);
        // Overide end

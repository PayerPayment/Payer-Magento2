...
class Tax extends \Magento\Sales\Model\Order\Invoice\Total\Tax
{
    ...
    public function collect(\Magento\Sales\Model\Order\Invoice $invoice)
    {
        ...
        if ($this->_canIncludeHandlingFee($invoice)) {
            $totalTax += $order->getHandlingFeeTaxAmount();
            $baseTotalTax += $order->getBaseHandlingFeeTaxAmount();
            $invoice->setHandlingFeeTaxAmount($order->getHandlingFeeTaxAmount());
            $invoice->setBaseHandlingFeeTaxAmount($order->getBaseHandlingFeeTaxAmount());
        }
        // end override
    ...
    /**
     * Check if HandlingFee tax calculation can be included to current invoice
     *
     * @param \Magento\Sales\Model\Order\Invoice $invoice
     * @return bool
     */
    protected function _canIncludeHandlingFee($invoice)
    {
        $includeHandlingFeeTax = true;
        /**
         * Check HandlingFee amount in previous invoices
         */
        foreach ($invoice->getOrder()->getInvoiceCollection() as $previousInvoice) {
            if ($previousInvoice->getHandlingFeeAmount() && !$previousInvoice->isCanceled()) {
                $includeHandlingFeeTax = false;
            }
        }

        return $includeHandlingFeeTax;
    }
}
